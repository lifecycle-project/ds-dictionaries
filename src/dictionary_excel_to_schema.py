...
dict_unit = pd.read_csv("https://data-catalogue.molgeniscloud.org/CatalogueOntologies/api/csv/Units")
dict_unit = dict_unit['name'].tolist()
dict_unit.append("")
dict_rep = 0

dict_new_version = "3_0"
dict_source_folder = "../dictionaries-source/lifecycle/" + dict_table + "/" + dict_new_version
...
variables_non_rep = pd.read_excel(excel_file, sheet_name='Variables', keep_default_na=False, na_values="",
                                  dtype={'name': str, 'valueType': str, 'unit': str, 'label': str})
categories_non_rep = pd.read_excel(excel_file, sheet_name='Categories', keep_default_na=False, na_values="",
...
variables_non_rep = variables_non_rep[["table", "name", "valueType", "entityType", "unit", "mimeType",
                                       "repeatable", "occurrenceGroup", "referencedEntityType", "index",
                                       "label", "alias"]]
...
categories_non_rep = categories_non_rep[["table", "variable", "name", "missing", "label"]]

# process repeat files
dict_rep = 1
excel_files = ["../dictionaries/core/2_4/2_4_monthly_rep.xlsx",
...
for excel_file in excel_files:
    variables = pd.read_excel(excel_file, sheet_name='Variables', keep_default_na=False, na_values="",
                              dtype={'name': str, 'valueType': str, 'unit': str, 'label': str})
    variables_rep = pd.concat([variables_rep, variables])
    categories = pd.read_excel(excel_file, sheet_name='Categories', keep_default_na=False, na_values="",
                               dtype={'variable': str, 'name': int, 'isMissing': bool, 'label': str})
...
variables_rep = variables_rep[["table", "name", "valueType", "entityType", "unit", "mimeType",
                               "repeatable", "occurrenceGroup", "referencedEntityType", "index",
                               "label", "alias"]]
...
# write csv(s)
variables.to_csv((dict_source_folder + "/Variables.csv"), index=False, quoting=csv.QUOTE_MINIMAL)

categories['missing'] = categories['missing'].astype(int)
categories.to_csv((dict_source_folder + "/Categories.csv"), index=False, quoting=csv.QUOTE_MINIMAL)
...
# write excel
writer = pd.ExcelWriter((dict_folder + "/" + dict_new_version + "_" + dict_table + ".xlsx"),
                        engine="xlsxwriter", )
