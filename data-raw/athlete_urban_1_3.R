library(readxl)
library(tidyverse)
# Copy over the original dictionary from dictionaries/urban_ath/1_3 to data-raw and rename folder to athlete_urban_1_3
non_rep_v <- readxl::read_xlsx("data-raw/athlete_urban_1_3/1_3_non_rep.xlsx", sheet = "Variables")
non_rep_c <- readxl::read_xlsx("data-raw/athlete_urban_1_3/1_3_non_rep.xlsx", sheet = "Categories")

trimester_rep_v <- readxl::read_xlsx("data-raw/athlete_urban_1_3/1_3_trimester_rep.xlsx", sheet = "Variables")
trimester_rep_c <- readxl::read_xlsx("data-raw/athlete_urban_1_3/1_3_trimester_rep.xlsx", sheet = "Categories")

yearly_rep_v <- readxl::read_xlsx("data-raw/athlete_urban_1_3/1_3_yearly_rep.xlsx", sheet = "Variables")
yearly_rep_c <- readxl::read_xlsx("data-raw/athlete_urban_1_3/1_3_yearly_rep.xlsx", sheet = "Categories")
...
athlete_urban_1_3 <- nest_join(variables, categories, by = "name")

# Update NEWS.md (changelogs)
# - Compare the old and new data variables and categories

data("athlete_urban_1_2")

diff <- dplyr::setdiff(athlete_urban_1_3, athlete_urban_1_2)
NEWS <- diff %>% dplyr::select(name, valueType, label)

write_lines(c("\n"), "NEWS.md", append = TRUE)
write_lines("## Release: athlete_urban_1_3", "NEWS.md", append = TRUE)
write_lines(c("\n"), "NEWS.md", append = TRUE)
write_lines("### Usage\n", "NEWS.md", append = TRUE)
write_lines(c("``` r\nlibrary(dsDictionaries)\ndata(athlete_urban_1_3)\n```\n"), "NEWS.md", append = TRUE)
write_lines("### Changes\n", "NEWS.md", append = TRUE)
write_lines(
  knitr::kable(NEWS, caption = "athlete_urban_1_3 changes"),
  "NEWS.md",
  append = TRUE, )
# Edit the NEWS.md file by hand to make sure it is formatted correctly

usethis::use_data(athlete_urban_1_3, overwrite = TRUE)
