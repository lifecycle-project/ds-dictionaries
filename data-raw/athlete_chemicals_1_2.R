library(readxl)
library(tidyverse)
# Copy over the original dictionary from dictionaries/chemicals_ath/1_2 to data-raw and rename folder to athlete_chemicals_1_2
non_rep_v <- readxl::read_xlsx("data-raw/athlete_chemicals_1_2/1_2_non_rep.xlsx", sheet = "Variables")
non_rep_c <- readxl::read_xlsx("data-raw/athlete_chemicals_1_2/1_2_non_rep.xlsx", sheet = "Categories")
...
categories <- tibble::add_column(table = "chemicals", categories, .before = "variable")
...
variables <- tibble::add_column(table = "chemicals", variables, .before = "name")
