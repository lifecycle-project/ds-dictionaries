library(readxl)
library(tidyverse)
# Copy over the original dictionary from dictionaries/outcome_ath/1_2 to data-raw and rename folder to athlete_outcome_1_2
non_rep_v <- readxl::read_xlsx("data-raw/athlete_outcome_1_2/1_2_non_rep.xlsx", sheet = "Variables")
non_rep_c <- readxl::read_xlsx("data-raw/athlete_outcome_1_2/1_2_non_rep.xlsx", sheet = "Categories")

monthly_rep_v <- readxl::read_xlsx("data-raw/athlete_outcome_1_2/1_2_monthly_rep.xlsx", sheet = "Variables")

trimester_rep_v <- readxl::read_xlsx("data-raw/athlete_outcome_1_2/1_2_trimester_rep.xlsx", sheet = "Variables")

yearly_rep_v <- readxl::read_xlsx("data-raw/athlete_outcome_1_2/1_2_yearly_rep.xlsx", sheet = "Variables")
yearly_rep_c <- readxl::read_xlsx("data-raw/athlete_outcome_1_2/1_2_yearly_rep.xlsx", sheet = "Categories")
...
diff <- dplyr::setdiff(athlete_outcome_1_2, athlete_outcome_1_1)
NEWS <- diff %>% dplyr::select(name, valueType, label)

write_lines(c("\n"), "NEWS.md", append = TRUE)
write_lines("## Release: athlete_outcome_1_2", "NEWS.md", append = TRUE)
write_lines(c("\n"), "NEWS.md", append = TRUE)
write_lines("### Usage\n", "NEWS.md", append = TRUE)
write_lines(c("``` r\nlibrary(dsDictionaries)\ndata(athlete_outcome_1_2)\n```\n"), "NEWS.md", append = TRUE)
write_lines("### Changes\n", "NEWS.md", append = TRUE)
write_lines(
  knitr::kable(NEWS, caption = "athlete_outcome_1_2 changes"),
  "NEWS.md",
  append = TRUE, )
# Edit the NEWS.md file by hand to make sure it is formatted correctly

usethis::use_data(athlete_outcome_1_2, overwrite = TRUE)
